...
if [[ -z "$VERSION_NAME" || -z "$VERSION_CODE" ]]; then
  echo "ERROR: versionName/versionCode not found in $VERSION_FILE" >&2
  exit 2
fi
...
if [[ "$ALLOW_NON_ASCII_COMMIT_SUBJECT" == "true" ]]; then
  echo "PASS: commit subject ascii check disabled"
else
  check_non_ascii_commit_subjects
fi
...
if [[ ! -f "$CHANGELOG_FILE" ]]; then
  echo "FAIL: missing changelog file ($CHANGELOG_FILE)" >&2
  FAIL=1
