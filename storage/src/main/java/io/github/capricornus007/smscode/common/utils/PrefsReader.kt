package io.github.capricornus007.smscode.common.utils
...
object PrefsReader {
    ...
    private fun getBooleanViaProvider(context: Context, key: String, defaultValue: Boolean): Boolean {
        return try {
            val uri = io.github.capricornus007.smscode.data.prefs.PrefsProvider.BOOL_URI.buildUpon()
    ...
    private fun getStringViaProvider(context: Context, key: String, defaultValue: String): String {
        return try {
            val uri = io.github.capricornus007.smscode.data.prefs.PrefsProvider.STRING_URI.buildUpon()
                .appendQueryParameter("key", key)
    ...
    private fun getIntViaProvider(context: Context, key: String, defaultValue: Int): Int {
        return try {
            val uri = io.github.capricornus007.smscode.data.prefs.PrefsProvider.INT_URI.buildUpon()
                .appendQueryParameter("key", key)
                .appendQueryParameter("default", defaultValue.toString())
                .build()
            context.contentResolver.query(uri, null, null, null, null)?.use { cursor ->
