...
object MobileEntitlementCoordinator {
    ...
    private val evaluationCache = TimedValueCache<MobileEntitlementEvaluation>(MOBILE_ENTITLEMENT_CACHE_TTL_MS)
    ...
    fun initialize(context: Context, scope: CoroutineScope) {
        ...
        scope.launch(Dispatchers.IO) { runCatching { refresh(context) } }
    }
    ...
    fun clearPendingChallenge(context: Context) = PrivateMobileEntitlementCoordinator.clearPendingChallenge(context)

    suspend fun refresh(context: Context, force: Boolean = false): MobileEntitlementEvaluation {
    ...
    suspend fun pollTelegramChallenge(context: Context, challengeId: String): MobileEntitlementActivationState =
        PrivateMobileEntitlementCoordinator.pollTelegramChallenge(context, challengeId).also { state ->
            state.evaluation?.let(::updateCache)
        }

    suspend fun activateWithLicenseCode(context: Context, licenseCode: String): MobileEntitlementEvaluation =
        PrivateMobileEntitlementCoordinator.activateWithLicenseCode(context, licenseCode).also(::updateCache)

    suspend fun activateByToken(context: Context, token: String): MobileEntitlementEvaluation =
    ...
    fun clearSavedLicenseCode(context: Context) = PrivateMobileEntitlementCoordinator.clearSavedLicenseCode(context)
