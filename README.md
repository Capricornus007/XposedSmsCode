...
如果你还需要将验证码、短信内容或应用通知进一步转发到 Telegram、Webhook、邮箱等通道，可以了解一下新项目 [信驿 Relay](https://github.com/Capricornus007/xinyi-relay)。它更偏向消息转发与验证码联动场景；若主要需求是验证码识别与自动输入，继续使用本项目会更合适。
...
- 支持官方只读验证码规则与自定义验证码匹配提取规则（支持导入与导出用户规则表）
- 在支持的应用中自动输入提取的验证码
- 提取并转发应用通知消息中的验证码
